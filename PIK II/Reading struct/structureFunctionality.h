...
void PrintProductInfo() // prints the structure
{
...
void writeBinaryFile(char *fileName)
{
    ...
    if (fwrite(&Info, sizeof(tsProduct), 1, fp) != 1)
    {
        printf("Error writing structure in WriteBinaryFile()\n");
        exit(EXIT_FAILURE); // check if there is error writing the structure into the file
    }
...
void readBinaryFileToStruct(char *fileName)
{
    ...
    fp = fopen(fileName, "rb"); // opens file for reading
    if (fp == NULL)
    {
    ...
    while (1)
    {
        if (fread(&Info, sizeof(tsProduct), 1, fp) != 1) // reads file data and save it to Info structure
        {
...
Node *makeNodeFromFile(char *FileName)
{
    ...
    while (1) // endless loop
    {
        ...
        if (fread(&temp->Info, sizeof(tsProduct), 1, fp) != 1) // reads info from file and fills it into the node that contains Info structure
        {
            break; // if no data is found exits the loop
        }
...
void printNode(char menuChoice, int MonthNumber, char *FileName)
{
    ...
    Max = Min = head->Info.price; // saves Min and Max value of the first element in the node

    while (head != NULL) // loops until node end is reached
    {
        Sum += head->Info.price; // sums all the price and save them to Sum variable
        if (head->Info.price > Max) // if the current price from node is larger than the first(max)
        {
            Max = head->Info.price; // max takes value of the bigger value in the node
        }
        else if (head->Info.price < Min) // if the current price from node is smaller than the first(min)
        {
            Min = head->Info.price; // min takes value of the smaller value in the node
        }
